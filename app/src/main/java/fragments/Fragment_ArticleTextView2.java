...
public class Fragment_ArticleTextView2 extends Fragment implements View.OnClickListener {
    ...
    public View onCreateView(LayoutInflater inflater, ViewGroup container, Bundle savedInstanceState) {
        return inflater.inflate(R.layout.fragment_article_text_view2, container, false);
    }
...
//        ((FrameLayout) getActivity().findViewById(R.id.frameLayout_bottom_bar)).setVisibility(View.VISIBLE);
    ...
    class TaskLoadContentPage extends AsyncTask<Integer , Void , Home_Pager_Page>{
        ...
        protected void onPostExecute(Home_Pager_Page page) {
            ...
            titrIds.add(tvTextTitle.getId());
            textIds.add(jText.getId());
            backId = nestedScrollView.getId();
            if(getActivity() != null)
                new TextAndBacks(getActivity()).changer(textIds, titrIds, nestedScrollView.getId());
            tvTitleToolbar.setVisibility(View.INVISIBLE);
            toolbarTitle = page.getTxtText();
            if(getActivity() != null) {
                collapsingToolbarLayout = (CollapsingToolbarLayout) getActivity().findViewById(R.id.fragment_article_collaps);
                ...
                tvTitleToolbar.setText(page.getTxtText());
            }
            ...
            appBarLayout.addOnOffsetChangedListener(new AppBarLayout.OnOffsetChangedListener() {
                ...
                public void onOffsetChanged(AppBarLayout appBarLayout, int offset) {
                    if (offset < -200) {
                        //tvTitleToolbar.setAnimation(AnimationUtils.loadAnimation(getActivity() , R.anim.from_middle));
                        tvTitleToolbar.setVisibility(View.VISIBLE);
                        tvBackToolbar.setVisibility(View.INVISIBLE);
                        relativeLayoutToolbarBack.setBackgroundColor(getResources().getColor(R.color.colorAccent));

                    } else {
                        //tvTitleToolbar.setAnimation(AnimationUtils.loadAnimation(getActivity(), R.anim.to_middle));
                        tvTitleToolbar.setVisibility(View.INVISIBLE);
                        tvBackToolbar.setVisibility(View.VISIBLE);
                        relativeLayoutToolbarBack.setBackgroundColor(getResources().getColor(android.R.color.transparent));
                    }
                }
            });
...
//                ).replace(false);
    ...
    private void gotoBuy(){
        ...
        TextView ivBtnAccept = (TextView) dialog.findViewById(R.id.dialog_sendMessage_text_accept);
        TextView ivBtnCancel = (TextView) dialog.findViewById(R.id.dialog_sendMessage_text_cancel);
